...
function ContractTab() {
  ...
  const doSave = async () => {
    setLoading(true);
    try { localStorage.setItem("sm:contract", JSON.stringify(contract)); } catch {}
    if (isCloud) {
      try {
        await api("DELETE", "shifts", { query: `store_id=eq.contract&date=eq.latest` });
        await api("POST", "shifts", { body: { store_id: "contract", emp_id: "system", date: "latest", start_time: JSON.stringify(contract), end_time: "0", break_min: 0 } });
      } catch {}
    }
    setSaved(true); setLoading(false);
    setTimeout(() => setSaved(false), 2000);
  ...
  useEffect(() => {
    (async () => {
      ...
      if (isCloud) {
        try {
          const res = await api("GET", "shifts", { query: `store_id=eq.contract&date=eq.latest` });
          if (res && res.length > 0) { try { data = JSON.parse(res[0].start_time); } catch {} }
        } catch {}
      }
      if (!data) { try { const v = localStorage.getItem("sm:contract"); if (v) data = JSON.parse(v); } catch {} }
  ...
  const S = {
    ...
    label: { fontSize: 13, color: "#5C4A35", fontWeight: 500 },
    input: { padding: "8px 10px", border: "1.5px solid #E0D5C5", borderRadius: 6, fontSize: 14, fontFamily: "inherit", outline: "none", background: "#FFFDF9", color: "#2D2016" },
  };

  return (
    <div>
      <div style={{ textAlign: "center", marginBottom: 16 }}>
        <div style={{ fontWeight: 700, fontSize: 16, color: "#2D2016" }}>근로계약서 작성</div>
        <div style={{ fontSize: 11, color: "#B5A48B" }}>입력 후 저장 → PDF 출력</div>
      </div>

      <div style={S.sec}>
        <div style={S.head}>👤 근로자 정보</div>
        <div style={{ display: "grid", gap: 10 }}>
          <div><div style={S.label}>이름</div><input value={contract.empName} onChange={e => u("empName", e.target.value)} placeholder="홍길동" style={{ ...S.input, width: "100%", marginTop: 4 }} /></div>
          <div><div style={S.label}>주소</div><input value={contract.empAddr} onChange={e => u("empAddr", e.target.value)} placeholder="서울시 중구..." style={{ ...S.input, width: "100%", marginTop: 4 }} /></div>
          <div><div style={S.label}>연락처</div><input value={contract.empPhone} onChange={e => u("empPhone", e.target.value)} placeholder="010-0000-0000" style={{ ...S.input, width: "100%", marginTop: 4 }} /></div>
        </div>
      </div>
      ...
      <div style={S.sec}>
        ...
        <div style={{ display: "grid", gap: 10 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            <div><div style={S.label}>계약 시작일</div><input type="date" value={contract.startDate} onChange={e => u("startDate", e.target.value)} style={{ ...S.input, width: "100%", marginTop: 4 }} /></div>
            <div><div style={S.label}>계약 종료일</div><input type="date" value={contract.endDate} onChange={e => u("endDate", e.target.value)} style={{ ...S.input, width: "100%", marginTop: 4 }} /></div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            <div><div style={S.label}>근로시간 시작</div><input type="time" value={contract.workStart} onChange={e => u("workStart", e.target.value)} style={{ ...S.input, width: "100%", marginTop: 4 }} /></div>
            <div><div style={S.label}>근로시간 종료</div><input type="time" value={contract.workEnd} onChange={e => u("workEnd", e.target.value)} style={{ ...S.input, width: "100%", marginTop: 4 }} /></div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            <div><div style={S.label}>휴게시간 시작</div><input type="time" value={contract.breakStart} onChange={e => u("breakStart", e.target.value)} style={{ ...S.input, width: "100%", marginTop: 4 }} /></div>
            <div><div style={S.label}>휴게시간 종료</div><input type="time" value={contract.breakEnd} onChange={e => u("breakEnd", e.target.value)} style={{ ...S.input, width: "100%", marginTop: 4 }} /></div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 8 }}>
            <div><div style={S.label}>근무요일</div><input value={contract.workDays} onChange={e => u("workDays", e.target.value)} placeholder="예: 월~금" style={{ ...S.input, width: "100%", marginTop: 4 }} /></div>
            <div><div style={S.label}>주 근무일수</div><input type="text" inputMode="numeric" value={contract.workDaysCount} onChange={e => u("workDaysCount", e.target.value.replace(/[^0-9]/g, ""))} placeholder="5" style={{ ...S.input, width: "100%", marginTop: 4 }} /></div>
          </div>
        </div>
      </div>
      ...
      <div style={S.sec}>
        ...
        <input value={contract.extraTask} onChange={e => u("extraTask", e.target.value)} placeholder="추가 업무 입력" style={{ ...S.input, width: "100%"}} />
      </div>
      ...
      <div style={S.sec}>
        ...
        <input type="date" value={contract.contractDate} onChange={e => u("contractDate", e.target.value)} style={{ ...S.input, width: "100%" }} />
      </div>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <B ghost onClick={doReset} style={{ padding: "14px 16px", fontSize: 14 }}>🔄 초기화</B>
      </div>

      <div style={{ marginTop: 12, padding: "10px 14px", background: "#F7F2EA", borderRadius: 10, fontSize: 11, color: "#A89878", lineHeight: 1.6 }}>
        <strong>고정값 (자동 적용)</strong><br />
        사업체: 팔팔너구리해장 · 주소: 서울 중구 퇴계로 421 1층 2호<br />
        대표자: 양두환, 박지현 · 근무장소: 팔팔너구리해장 신당본점<br />
        임금지급일: 익월 10일 · 업무: 홀서빙, 청소, 기타
      </div>
    </div>
  );
}
