...
function DailyReportTab() {
  ...
  const numStyle = { width: 130, padding: "6px 10px", border: "1.5px solid #E0D5C5", borderRadius: 6, fontSize: 14, fontFamily: "inherit", textAlign: "right", outline: "none", background: "#FFFDF9" };
  const txtStyle = { width: "100%", padding: "8px 10px", border: "1.5px solid #E0D5C5", borderRadius: 8, fontSize: 13, fontFamily: "inherit", outline: "none", resize: "vertical", background: "#FFFDF9", boxSizing: "border-box" };

  const NR = (label, field, unit) => (
    <div key={field} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #F5F0E8" }}>
      <span style={{ fontSize: 13, color: "#5C4A35", fontWeight: 500 }}>{label}</span>
      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
        <input type="text" inputMode="numeric" value={report[field]} onChange={e => u(field, e.target.value.replace(/[^0-9]/g, ""))} placeholder="0" style={numStyle} />
        <span style={{ fontSize: 12, color: "#8B7355" }}>{unit}</span>
      </div>
    </div>
  );

  const TR = (label, field, placeholder) => (
    <div key={field} style={{ marginBottom: 10 }}>
      {label && <div style={{ fontSize: 12, color: "#8B7355", fontWeight: 500, marginBottom: 4 }}>{label}</div>}
      <textarea value={report[field]} onChange={e => u(field, e.target.value)} placeholder={placeholder || "내용 입력"} rows={2} style={txtStyle} />
    </div>
  );
    ...
    <div>
      ...
      {loading ? (
        ...
        <React.Fragment>
          <Sec title="💰 매출">
            {NR("총 매출", "sales", "원")}
            {NR("영수증 리뷰 수", "reviews", "건")}
          </Sec>

          <Sec title="📦 중요 입고">
            {TR("", "incoming", "예: 우동사리, 쌀, 유부")}
          </Sec>

          <Sec title="🏪 매장 분위기">
            {NR("점심 매출", "lunchSales", "만원")}
            {TR("점심 분위기", "lunchMood", "점심 시간대 분위기를 적어주세요")}
            {NR("저녁 매출", "dinnerSales", "만원")}
            {TR("저녁 분위기", "dinnerMood", "저녁 시간대 분위기를 적어주세요")}
            {TR("유입 경로 (추측)", "trafficSource", "예: 동네 주민, 배달 앱, SNS 등")}
          </Sec>
