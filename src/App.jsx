...
function nC(s, e) { const [sh, sm] = s.split(":").map(Number), [eh, em] = e.split(":").map(Number); let n = 0, c = sh * 60 + sm, end = eh * 60 + em; if (end <= c) end += 1440; while (c < end) { const h = Math.floor(c / 60) % 24; if (h >= 22 || h < 6) n++; c++; } return n / 60; }
...
function exportPDF(emps, shifts, yr, mo) {
  const { last } = mI(yr, mo);
  ...
  const wageData = emps.map(emp => {
    ...
    return { name: emp.name, wage: emp.wage, phone: emp.phone, bank: emp.bank, days, tH: Math.round(tH * 10) / 10, base, weekly, total: base + weekly };
  });
...
<table>
  <tr><th>이름</th><th>근무일수</th><th>총 근무시간</th><th>기본급</th><th>주휴수당</th><th>총 급여</th></tr>
  ${wageRows}
</table>
...
<div class="footer">가맹점 매니저 · ${docDate} 출력</div>
</body></html>`;
  ...
  let empRows = emps.map((emp, idx) => {
    ...
    for (let d = 1; d <= last; d++) {
      ...
      if (sh) {
        ...
        const period = startH < 12 ? '오전' : startH < 17 ? '오후' : '저녁';
        cells += `<td class="dc" style="background:${bg};font-size:10px;line-height:1.4"><strong>${hrs}</strong><br><span style="color:#999;font-size:9px">${period}</span></td>`;
      } else {
...
<div class="header">
  <div>
    <h1>${yr}년 ${mo + 1}월 근무 스케줄</h1>
    <span class="sub">직원 ${emps.length}명</span>
  </div>
...
<div class="footer">☕ 가맹점 매니저 · 근무시간은 휴게시간 제외 기준</div>
</body></html>`;
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
        ...
        <B small ghost onClick={() => exportPDF(emps, shifts, yr, mo)}>💰 급여 리포트</B>
      </div>
...
function WageTab({ emps, shifts }) {
  ...
  const calc = emp => {
    let tH = 0, nH = 0, days = 0; const wm = {};
    for (let d = 1; d <= last; d++) {
      const sh = gS(emp.id, dkF(yr, mo, d)); if (!sh) continue;
      days++; const hrs = hC(sh.start_time, sh.end_time, sh.break_min); tH += hrs; nH += nC(sh.start_time, sh.end_time);
      const wk = wK(yr, mo, d); wm[wk] = (wm[wk] || 0) + hrs;
    }
    const base = Math.round(tH * emp.wage), night = Math.round(nH * emp.wage * 0.5);
    let weekly = 0; Object.values(wm).forEach(wh => { if (wh >= 15) weekly += Math.round((wh / 5) * emp.wage); });
    return { tH: Math.round(tH * 10) / 10, nH: Math.round(nH * 10) / 10, days, base, night, weekly, total: base + night + weekly };
  };
  ...
  return (
    <div>
                ...
                {d.days === 0 ? <p style={{ color: "#C4B99A", fontSize: 13, margin: 0 }}>등록된 근무일정 없음</p> : (
                  <React.Fragment>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(95px,1fr))", gap: 8, marginBottom: 12 }}>
                      {[{ l: "근무일", v: `${d.days}일` }, { l: "총시간", v: `${d.tH}h` }, { l: "야간", v: `${d.nH}h` }, { l: "기본급", v: d.base.toLocaleString() }, { l: "야간수당", v: d.night.toLocaleString() }, { l: "주휴수당", v: d.weekly.toLocaleString() }].map(x => (
      ...
      <div style={{ marginTop: 16, padding: "12px 16px", background: "#F7F2EA", borderRadius: 10, fontSize: 11, color: "#A89878", lineHeight: 1.8 }}>
        <strong>계산 기준</strong> · 실제 일정 기반 · 야간(22~06시) 50% 가산 · 주휴: 일~토 한 주 15h 이상 시 지급 · 최저시급 {MW.toLocaleString()}원
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "100vh", fontFamily: "'Noto Sans KR', sans-serif", background: "#F5EFE6" }}>
      <div style={{ background: "#FFFDF9", borderRadius: 20, border: "1px solid #EBE2D5", padding: "40px 32px", textAlign: "center", width: 300, maxWidth: "90vw" }}>
        ...
        <h2 style={{ margin: "0 0 6px", fontSize: 18, fontWeight: 800, color: "#2D2016" }}>가맹점 매니저</h2>
    ...
    <div style={{ fontFamily: "'Noto Sans KR', sans-serif", background: "#F5EFE6", minHeight: "100vh", color: "#2D2016" }}>
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "16px 12px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
          ...
          <div>
            <h1 style={{ margin: 0, fontSize: 18, fontWeight: 800, letterSpacing: -0.5 }}>가맹점 매니저</h1>
