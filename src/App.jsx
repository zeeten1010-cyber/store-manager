...
function EmpPanel({ emps, setEmps, sel, setSel, persist }) {
  ...
  const doDelete = async (id) => {
    ...
    const hasShifts = shifts.some(s => s.emp_id === id);
    let msg = `"${emp?.name}" 직원을 삭제하시겠습니까?`;
    if (hasShifts) msg += "\n\n⚠️ 이 직원의 스케줄 데이터도 함께 삭제됩니다.";
    if (!confirm(msg)) return;
    setBusy(true);
    if (isCloud) {
      try { await api("DELETE", "employees", { query: `id=eq.${id}` }); } catch {}
      if (hasShifts) { try { await api("DELETE", "shifts", { query: `store_id=eq.${SID}&emp_id=eq.${id}` }); } catch {} }
    }
