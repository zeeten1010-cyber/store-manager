...
function EmpPanel({ emps, setEmps, sel, setSel, persist }) {
  ...
  const doDelete = async (id) => {
    ...
    if (sel === id) { const vis = emps.filter(e => e.id !== id); setSel(vis[0]?.id || null); }
    setMode(null);
    // 강제 리렌더
    setEmps([...emps]);
  };
  ...
  const saveBonus = async (empId, val, memo) => {
    ...
    if (isCloud) {
      ...
      try {
        ...
        await api("POST", "shifts", { body: { store_id: "bonus", emp_id: empId, date: dateKey, start_time: String(val || 0), end_time: memo || "", break_min: 0 } });
      } catch {}
    }
  };
                  ...
                  <React.Fragment>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(95px,1fr))", gap: 8, marginBottom: 12 }}>
                      ...
                      <div style={{ padding: "8px 10px", background: "#EDF7ED", borderRadius: 8 }}>
                        ...
                        <input type="number" value={getBonus(emp.id) || ""} onChange={e => saveBonus(emp.id, e.target.value, getMemo(emp.id))}
                          placeholder="0" style={{ width: "100%", fontSize: 13, fontWeight: 700, border: "1px solid #C5DFC5", borderRadius: 4, padding: "2px 6px", fontFamily: "inherit", background: "#FBFFF9", color: "#2D2016", outline: "none", marginBottom: 4 }} />
                        <input type="text" value={getMemo(emp.id)} onChange={e => saveBonus(emp.id, getBonus(emp.id), e.target.value)}
