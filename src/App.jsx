...
function exportPDF(emps, shifts, yr, mo, bonuses, memos) {
  const { last } = mI(yr, mo);
  ...
  const wageData = emps.map(emp => {
    ...
    const memo = (memos || {})[bKey(emp.id)] || "";
    return { name: emp.name, ssn: emp.ssn, wage: emp.wage, phone: emp.phone, bank: emp.bank, days, tH: Math.round(tH * 10) / 10, base, weekly, bonus, memo, total: base + weekly + bonus };
  });
  const activeData = wageData.filter(d => d.days > 0);
  const grand = activeData.reduce((s, d) => s + d.total, 0);

  // 급여 행 (근무한 직원만)
  let wageRows = activeData.map(d => `
    <tr>
      <td class="tc" style="font-weight:600">${d.name}</td>
      <td class="tc" style="font-size:10px">${d.ssn || ''}</td>
      ...
      <td class="tc" style="font-size:10px">${d.memo || '-'}</td>
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  th { padding: 8px 6px; background: #111; color: #fff; font-size: 10px; font-weight: 600; text-align: center; }
  td { padding: 7px 6px; border: 1px solid #E0E0E0; font-size: 11px; }
...
<div class="title-section">
  ...
  <p>근무 직원 ${activeData.length}명 · 기준 시급 ${fmt(MW)}원</p>
</div>
...
<div class="info-box">
  ...
  <div><div class="label">근무 직원</div><div class="value">${activeData.length}명</div></div>
  <div><div class="label">출력일</div><div class="value">${docDate}</div></div>
</div>

<div class="section-title"><span class="bar"></span> 급여 계산</div>
<table>
  <tr><th>이름</th><th>주민번호</th><th>근무일수</th><th>총 근무시간</th><th>기본급</th><th>주휴수당</th><th>기타수당</th><th>메모</th><th>총 급여</th></tr>
  ${wageRows}
</table>
...
function exportSchedulePDF(emps, shifts, yr, mo) {
  ...
  const empTotals = emps.map(emp => {
    ...
    for (let d = 1; d <= last; d++) {
      ...
      shs.forEach(sh => { t += hC(sh.start_time, sh.end_time, sh.break_min); });
    }
  ...
  let empRows = emps.map((emp, idx) => {
    ...
    for (let d = 1; d <= last; d++) {
      ...
      if (shs.length > 0) {
        const cellContent = shs.map(sh => {
          const hrs = hC(sh.start_time, sh.end_time, sh.break_min);
          return `<div style="line-height:1.3"><div style="font-size:8px;font-weight:700">${fmtTime(sh.start_time)}</div><div style="font-size:8px;color:#999">~${fmtTime(sh.end_time)}</div><div style="font-size:7px;color:#C4956A">(${hrs}h)</div></div>`;
        }).join('');
        cells += `<td class="dc" style="background:${bg};padding:2px 1px">${cellContent}</td>`;
      } else {
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: '맑은 고딕', 'Malgun Gothic', 'Noto Sans KR', sans-serif; font-size: 10px; line-height: 1.4; color: #000; }
  h1 { text-align: center; font-size: 18px; letter-spacing: 10px; margin-bottom: 12px; padding-bottom: 8px; }
  .intro { margin-bottom: 10px; font-size: 10px; line-height: 1.6; }
  .section { margin-bottom: 8px; }
  .section-title { font-size: 11px; font-weight: bold; margin-bottom: 4px; }
  table { width: 100%; border-collapse: collapse; margin-bottom: 2px; }
  td, th { border: 1px solid #000; padding: 4px 8px; font-size: 10px; vertical-align: top; }
  .th { background: #F5F5F5; font-weight: bold; width: 25%; text-align: center; vertical-align: middle; }
  .note { font-size: 9px; color: #555; margin-top: 2px; margin-left: 2px; }
  .fixed-text { margin-bottom: 8px; font-size: 10px; line-height: 1.6; padding-left: 8px; }
...
function EmpPanel({ emps, setEmps, sel, setSel, shifts, setShifts, persist }) {
  ...
  const doDelete = async (id) => {
    ...
    if (!confirm(`"${emp?.name}" 직원을 목록에서 숨기시겠습니까?\n\n※ 이전 스케줄과 급여 데이터는 유지됩니다.`)) return;
    try {
      const hidden = JSON.parse(localStorage.getItem("sm:hidden-emps") || "[]");
      if (!hidden.includes(id)) hidden.push(id);
      localStorage.setItem("sm:hidden-emps", JSON.stringify(hidden));
    } catch {}
    if (sel === id) { const vis = emps.filter(e => e.id !== id && !hiddenIds.includes(e.id)); setSel(vis[0]?.id || null); }
    setMode(null); setEmps([...emps]);
  };

  const doPermDelete = async (id) => {
    const emp = emps.find(e => e.id === id);
    if (!confirm(`⚠️ "${emp?.name}" 직원을 완전 삭제하시겠습니까?\n\n모든 스케줄과 급여 데이터가 영구 삭제됩니다!\n이 작업은 되돌릴 수 없습니다.`)) return;
    setBusy(true);
    if (isCloud) {
      try { await api("DELETE", "employees", { query: `id=eq.${id}` }); } catch {}
      try { await api("DELETE", "shifts", { query: `store_id=eq.${SID}&emp_id=eq.${id}` }); } catch {}
    }
    // 숨김 목록에서도 제거
    try {
      const hidden = JSON.parse(localStorage.getItem("sm:hidden-emps") || "[]");
      localStorage.setItem("sm:hidden-emps", JSON.stringify(hidden.filter(h => h !== id)));
    } catch {}
    const nx = emps.filter(e => e.id !== id);
    const nxS = shifts.filter(s => s.emp_id !== id);
    setEmps(nx); localSet("sm:emp", nx);
    setShifts(nxS); localSet("sm:sh", nxS);
    if (sel === id) setSel(nx[0]?.id || null);
    setBusy(false); setMode(null); setShowHidden(false);
  };
  ...
  const hiddenIds = (() => { try { return JSON.parse(localStorage.getItem("sm:hidden-emps") || "[]"); } catch { return []; } })();
  const visibleEmps = emps.filter(e => !hiddenIds.includes(e.id));
  const hiddenEmps = emps.filter(e => hiddenIds.includes(e.id));
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12, alignItems: "center" }}>
        ...
        {hiddenEmps.length > 0 && (
          <button onClick={() => setShowHidden(!showHidden)} style={{ padding: "7px 14px", borderRadius: 20, border: "1.5px solid #E0D5C5", background: showHidden ? "#F5E0DE" : "transparent", cursor: "pointer", fontFamily: "inherit", fontWeight: 500, fontSize: 12, color: "#B5A48B" }}>
            {showHidden ? "✕ 닫기" : `👁 숨긴 직원 (${hiddenEmps.length})`}
          </button>
        )}
      </div>

      {showHidden && hiddenEmps.length > 0 && (
        <div style={{ background: "#FFF8F6", borderRadius: 14, border: "1px solid #F0E0D8", padding: 14, marginBottom: 12 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: "#C4956A", marginBottom: 10 }}>숨긴 직원</div>
          {hiddenEmps.map(emp => (
            <div key={emp.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #F0EBE3" }}>
              <button onClick={() => setSel(emp.id)} style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "none", border: "none", cursor: "pointer", fontFamily: "inherit", fontWeight: 600, fontSize: 13, color: "#999" }}>
                <span style={{ width: 8, height: 8, borderRadius: "50%", background: emp.color, opacity: 0.5 }} />{emp.name}
              </button>
              <div style={{ display: "flex", gap: 6 }}>
                <button onClick={() => doRestore(emp.id)} style={{ padding: "4px 10px", borderRadius: 6, border: "1px solid #5CB85C", background: "#EFFFEF", cursor: "pointer", fontFamily: "inherit", fontSize: 11, fontWeight: 600, color: "#5CB85C" }}>복원</button>
                <button onClick={() => doPermDelete(emp.id)} style={{ padding: "4px 10px", borderRadius: 6, border: "1px solid #D95550", background: "#FFF5F5", cursor: "pointer", fontFamily: "inherit", fontSize: 11, fontWeight: 600, color: "#D95550" }}>완전삭제</button>
              </div>
            </div>
          ))}
        </div>
...
function ScheduleTab({ emps, shifts, setShifts, sel }) {
  ...
  const saveS = async (day) => {
    if (!sel || busy) return;
    if (!ss || !se) { alert("시작/종료 시간을 입력해주세요"); return; }
    setBusy(true);
    ...
    if (isCloud) {
      try {
        await api("POST", "shifts", { body: ns });
        cloudOk = true;
      } catch (err) {
        // unique 제약 에러일 수 있음 - 기존 삭제 후 재시도
        try {
          await api("DELETE", "shifts", { query: `store_id=eq.${SID}&emp_id=eq.${sel}&date=eq.${ds}&start_time=eq.${ss}` });
          await api("POST", "shifts", { body: ns });
          cloudOk = true;
        } catch { cloudOk = false; }
      }
    }
    const nx = [...shifts.filter(s => !(s.emp_id === sel && s.date === ds && s.start_time === ss)), ns];
  ...
  const rmS = async (day, startTime) => {
    ...
    setBusy(true);
    const ds = dkF(yr, mo, day);
    if (isCloud) { try { await api("DELETE", "shifts", { query: `store_id=eq.${SID}&emp_id=eq.${sel}&date=eq.${ds}&start_time=eq.${startTime}` }); } catch {} }
    let found = false;
  ...
  const rmAny = async (day, empId, startTime) => {
    if (busy) return;
    setBusy(true);
    const ds = dkF(yr, mo, day);
    if (isCloud) { try { await api("DELETE", "shifts", { query: `store_id=eq.${SID}&emp_id=eq.${empId}&date=eq.${ds}&start_time=eq.${startTime}` }); } catch {} }
    let found = false;
    const nx = shifts.filter(s => {
      if (!found && s.emp_id === empId && s.date === ds && s.start_time === startTime) { found = true; return false; }
      return true;
    });
    setShifts(nx); localSet("sm:sh", nx);
    setBusy(false);
  ...
  const rmAllS = async (day) => {
    ...
    setBusy(true);
    const ds = dkF(yr, mo, day);
    if (isCloud) { try { await api("DELETE", "shifts", { query: `store_id=eq.${SID}&emp_id=eq.${sel}&date=eq.${ds}` }); } catch {} }
                ...
                {editing ? (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                    {allSh.map(({ emp, sh }, idx) => {
                      const hrs = hC(sh.start_time, sh.end_time, sh.break_min);
                      return (
                        <div key={`${emp.id}-${idx}`} style={{ fontSize: 10, padding: "2px 4px", borderRadius: 4, background: emp.color + "20", color: emp.color, fontWeight: 600, border: emp.id === sel ? `1.5px solid ${emp.color}` : "none", overflow: "hidden", lineHeight: 1.4, wordBreak: "break-all", display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                          <span>
                            <strong>{emp.name}</strong> {sh.start_time}~{sh.end_time}
                            <span style={{ fontSize: 9, opacity: 0.7 }}> ({hrs}h)</span>
                          </span>
                          <button onClick={e => { e.stopPropagation(); rmAny(day, emp.id, sh.start_time); }} style={{ background: "none", border: "none", color: emp.color, cursor: "pointer", fontSize: 10, padding: "0 2px", opacity: 0.5, flexShrink: 0 }}>✕</button>
                        </div>
                      );
                    })}
                  </div>
...
function WageTab({ emps, shifts }) {
    ...
    (async () => {
      if (isCloud) {
        try {
          ...
          if (data) {
            const b = {}, m = {};
            data.forEach(r => { b[`${r.emp_id}|${r.date}`] = Number(r.start_time) || 0; m[`${r.emp_id}|${r.date}`] = r.end_time || ""; });
            setBonuses(b); setMemos(m);
          }
        } catch {}
      } else {
        try { const b = localStorage.getItem("sm:bonus"); if (b) setBonuses(JSON.parse(b)); } catch {}
        try { const m = localStorage.getItem("sm:bonus-memo"); if (m) setMemos(JSON.parse(m)); } catch {}
      }
    })();
  }, []);
  ...
  const getBonusRaw = (empId) => bonuses[bonusKey(empId)] ?? "";
  const getMemo = (empId) => memos[bonusKey(empId)] || "";
  const saveBonus = async (empId, val, memo) => {
    const bk = bonusKey(empId);
    const nb = { ...bonuses, [bk]: val };
    const nm = { ...memos, [bk]: memo || "" };
    setBonuses(nb); setMemos(nm);
    try { localStorage.setItem("sm:bonus", JSON.stringify(nb)); } catch {}
    try { localStorage.setItem("sm:bonus-memo", JSON.stringify(nm)); } catch {}
    const numVal = Number(val) || 0;
    if (isCloud && !isNaN(numVal)) {
      const dateKey = `${yr}-${mo + 1}`;
      try {
        await api("DELETE", "shifts", { query: `store_id=eq.bonus&emp_id=eq.${empId}&date=eq.${dateKey}` });
        await api("POST", "shifts", { body: { store_id: "bonus", emp_id: empId, date: dateKey, start_time: String(numVal), end_time: memo || "", break_min: 0 } });
      } catch {}
    }
  };
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 12, display: "flex", gap: 8, flexWrap: "wrap" }}>
        <B small ghost onClick={() => exportSchedulePDF(emps, shifts, yr, mo)}>📅 스케줄표</B>
        <B small ghost onClick={() => exportPDF(emps, shifts, yr, mo, bonuses, memos)}>💰 급여 리포트</B>
      </div>
                  ...
                  <React.Fragment>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(95px,1fr))", gap: 8, marginBottom: 12 }}>
                      ...
                      <div style={{ padding: "8px 10px", background: "#EDF7ED", borderRadius: 8 }}>
                        ...
                        <input type="text" inputMode="numeric" value={getBonusRaw(emp.id)} 
                          onChange={e => saveBonus(emp.id, e.target.value, getMemo(emp.id))}
                          onBlur={e => { try { const v = e.target.value.replace(/[^0-9+\-*/().]/g, ""); if (v && /[+\-*/]/.test(v)) { const r = Function('"use strict"; return (' + v + ')')(); if (!isNaN(r)) saveBonus(emp.id, String(Math.round(r)), getMemo(emp.id)); } } catch {} }}
                          placeholder="예: +30000-20000" style={{ width: "100%", fontSize: 13, fontWeight: 700, border: "1px solid #C5DFC5", borderRadius: 4, padding: "2px 6px", fontFamily: "inherit", background: "#FBFFF9", color: "#2D2016", outline: "none", marginBottom: 4 }} />
                        <input type="text" value={getMemo(emp.id)} onChange={e => saveBonus(emp.id, getBonusRaw(emp.id), e.target.value)}
                          placeholder="메모" style={{ width: "100%", fontSize: 11, border: "1px solid #C5DFC5", borderRadius: 4, padding: "2px 6px", fontFamily: "inherit", background: "#FBFFF9", color: "#6B9E6B", outline: "none" }} />
                      </div>
                    </div>
...
/* ═══════ 근로계약서 ═══════ */
const CONTRACT_EMPTY = { empName: "", empAddr: "", empPhone: "", startDate: "", endDate: "", workStart: "", workEnd: "", breakStart: "", breakEnd: "", workDays: "", workDaysCount: "", wage: "10320", insEmp: false, insInd: false, insPen: false, insHth: false, extraTask: "", contractDate: "" };
...
function ContractTab() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (isCloud) {
        try {
          const res = await api("GET", "shifts", { query: `store_id=eq.contract&order=date.desc` });
          if (res) list = res.map(r => { try { return { id: r.date, ...JSON.parse(r.start_time) }; } catch { return null; } }).filter(Boolean);
        } catch {}
      }
      if (list.length === 0) {
        try { const v = localStorage.getItem("sm:contracts"); if (v) list = JSON.parse(v); } catch {}
      }
      setContracts(list);
    })();
  }, []);
  ...
  const doSave = async () => {
    if (!contract.empName.trim()) { alert("근로자 이름을 입력해주세요"); return; }
    setLoading(true);
    const id = current || ("ct_" + Date.now());
    const data = { ...contract, id };
    if (isCloud) {
      try {
        await api("DELETE", "shifts", { query: `store_id=eq.contract&date=eq.${id}` });
        await api("POST", "shifts", { body: { store_id: "contract", emp_id: "system", date: id, start_time: JSON.stringify(data), end_time: "0", break_min: 0 } });
      } catch {}
    }
    const nx = current ? contracts.map(c => c.id === current ? data : c) : [data, ...contracts];
    setContracts(nx);
    try { localStorage.setItem("sm:contracts", JSON.stringify(nx)); } catch {}
    setCurrent(id); setSaved(true); setLoading(false);
    setTimeout(() => { setSaved(false); setView("list"); }, 1500);
  };

  const doDelete = async (id) => {
    const c = contracts.find(c => c.id === id);
    if (!confirm(`"${c?.empName}" 계약서를 삭제하시겠습니까?`)) return;
    if (isCloud) { try { await api("DELETE", "shifts", { query: `store_id=eq.contract&date=eq.${id}` }); } catch {} }
    const nx = contracts.filter(c => c.id !== id);
    setContracts(nx);
    try { localStorage.setItem("sm:contracts", JSON.stringify(nx)); } catch {}
    if (current === id) { setCurrent(null); setContract(CONTRACT_EMPTY); }
  };

  const doNew = () => { setCurrent(null); setContract(CONTRACT_EMPTY); setSaved(false); setView("edit"); };
  const doOpen = (c) => { setCurrent(c.id); setContract({ ...CONTRACT_EMPTY, ...c }); setSaved(false); setView("edit"); };

  const S = {
    sec: { background: "#FFFDF9", borderRadius: 14, border: "1px solid #EBE2D5", padding: 16, marginBottom: 12 },
    head: { fontSize: 13, fontWeight: 700, color: "#C4956A", marginBottom: 12, paddingBottom: 6, borderBottom: "1px solid #F0EBE3" },
    row: { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #F5F0E8" },
    label: { fontSize: 13, color: "#5C4A35", fontWeight: 500, marginBottom: 4 },
    input: { padding: "8px 10px", border: "1.5px solid #E0D5C5", borderRadius: 6, fontSize: 14, fontFamily: "inherit", outline: "none", background: "#FFFDF9", color: "#2D2016", width: "100%" },
  };

  if (view === "list") return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <div style={{ fontWeight: 700, fontSize: 16, color: "#2D2016" }}>📄 근로계약서</div>
        <B primary onClick={doNew}>+ 새 계약서</B>
      </div>
      {contracts.length === 0 ? (
        <div style={{ textAlign: "center", padding: 40, color: "#B5A48B" }}>
          <div style={{ fontSize: 32, marginBottom: 12 }}>📄</div>
          <div style={{ marginBottom: 16 }}>저장된 계약서가 없습니다</div>
          <B primary onClick={doNew}>첫 계약서 작성하기</B>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {contracts.map(c => (
            <div key={c.id} style={{ background: "#FFFDF9", borderRadius: 14, border: "1px solid #EBE2D5", padding: "14px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div onClick={() => doOpen(c)} style={{ cursor: "pointer", flex: 1 }}>
                <div style={{ fontWeight: 700, fontSize: 15, color: "#2D2016", marginBottom: 4 }}>{c.empName || "이름 없음"}</div>
                <div style={{ fontSize: 12, color: "#B5A48B" }}>
                  {c.startDate ? c.startDate + " ~" : ""}{c.endDate ? " " + c.endDate : ""}
                  {c.wage ? " · 시급 " + Number(c.wage).toLocaleString() + "원" : ""}
                </div>
              </div>
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
        <button onClick={() => setView("list")} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 20, color: "#8B7355", padding: 0 }}>←</button>
        <div style={{ fontWeight: 700, fontSize: 16, color: "#2D2016" }}>{current ? "계약서 수정" : "새 계약서 작성"}</div>
      </div>
      ...
      <div style={S.sec}>
        <div style={S.head}>📋 계약 조건</div>
        <div style={{ display: "grid", gap: 10 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            <div><div style={S.label}>계약 시작일</div><input type="date" value={contract.startDate} onChange={e => u("startDate", e.target.value)} style={S.input} /></div>
            <div><div style={S.label}>계약 종료일</div><input type="date" value={contract.endDate} onChange={e => u("endDate", e.target.value)} style={S.input} /></div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            <div><div style={S.label}>근로시간 시작</div><input type="time" value={contract.workStart} onChange={e => u("workStart", e.target.value)} style={S.input} /></div>
            <div><div style={S.label}>근로시간 종료</div><input type="time" value={contract.workEnd} onChange={e => u("workEnd", e.target.value)} style={S.input} /></div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            <div><div style={S.label}>휴게시간 시작</div><input type="time" value={contract.breakStart} onChange={e => u("breakStart", e.target.value)} style={S.input} /></div>
            <div><div style={S.label}>휴게시간 종료</div><input type="time" value={contract.breakEnd} onChange={e => u("breakEnd", e.target.value)} style={S.input} /></div>
          </div>
          <div>
            <div style={S.label}>근무요일</div>
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              {["월","화","수","목","금","토","일"].map(day => {
                const sel2 = (contract.workDays || "").includes(day);
                return <button key={day} onClick={() => { const cur = contract.workDays || ""; const days = cur ? cur.split(",").filter(Boolean) : []; const nx = sel2 ? days.filter(d => d !== day) : [...days, day]; u("workDays", nx.join(",")); }} style={{ padding: "6px 12px", borderRadius: 8, border: sel2 ? "2px solid #C4956A" : "1.5px solid #E0D5C5", background: sel2 ? "#C4956A" : "#F5F0E8", color: sel2 ? "#FFF" : "#8B7355", fontWeight: 600, fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>{day}</button>;
              })}
            </div>
          </div>
      ...
      <div style={S.sec}>
        ...
        <div style={S.row}>
          <span style={S.label}>시급</span>
          <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
            <input type="text" inputMode="numeric" value={contract.wage} onChange={e => u("wage", e.target.value.replace(/[^0-9]/g,""))} placeholder="10320" style={{ ...S.input, width: 100, textAlign: "right" }} />
            <span style={{ fontSize: 12, color: "#8B7355" }}>원</span>
          </div>
        </div>
        <div style={{ fontSize: 11, color: "#B5A48B", marginTop: 6 }}>※ 2026년 최저임금: 시간급 10,320원</div>
      </div>

      <div style={S.sec}>
        <div style={S.head}>📝 업무내용</div>
        <div style={{ fontSize: 13, color: "#5C4A35", marginBottom: 8 }}>홀서빙, 청소, 기타:</div>
      ...
      <div style={S.sec}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          {[["고용보험","insEmp"],["산재보험","insInd"],["국민연금","insPen"],["건강보험","insHth"]].map(([label, key]) => (
            <label key={key} onClick={() => uBool(key)} style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", fontSize: 14, color: "#2D2016" }}>
              <span style={{ width: 22, height: 22, borderRadius: 5, border: `2px solid ${contract[key] ? "#C4956A" : "#D5C9B8"}`, background: contract[key] ? "#C4956A" : "transparent", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 12, color: "#FFF", fontWeight: 700, flexShrink: 0 }}>{contract[key] ? "✓" : ""}</span>
              {label}
            </label>
          ))}
        </div>
      ...
      <div style={{ marginTop: 12, padding: "10px 14px", background: "#F7F2EA", borderRadius: 10, fontSize: 11, color: "#A89878", lineHeight: 1.6 }}>
        <strong>고정값</strong> · 사업체: 팔팔너구리해장 · 주소: 서울 중구 퇴계로 421 1층 2호 · 대표자: 양두환, 박지현 · 임금지급일: 익월 10일
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setShifts(s || []);
      if (e && e.length > 0) {
        const hidden = (() => { try { return JSON.parse(localStorage.getItem("sm:hidden-emps") || "[]"); } catch { return []; } })();
        const visible = e.filter(emp => !hidden.includes(emp.id));
        setSel(visible[0]?.id || e[0].id);
      }
      setReady(true);
    })();
  }, []);
  ...
  return (
    <div style={{ fontFamily: "'Noto Sans KR', sans-serif", background: "#F5EFE6", minHeight: "100vh", color: "#2D2016" }}>
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "16px 8px" }}>
        ...
        {tab !== "checklist" && tab !== "report" && tab !== "contract" && <EmpPanel emps={emps} setEmps={setEmps} sel={sel} setSel={setSel} shifts={shifts} setShifts={setShifts} />}
