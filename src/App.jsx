...
const SID = "default";
const MW = 10320;
const COLORS = ["#E05D44","#3B82F6","#22C55E","#A855F7","#F59E0B","#06B6D4","#EC4899","#84CC16","#F97316","#6366F1","#14B8A6","#E11D48","#8B5CF6","#0EA5E9","#D97706","#10B981"];
                ...
                {editing ? (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                    {allSh.map(({ emp, sh }) => {
                      ...
                      return (
                        <div key={emp.id} style={{ fontSize: 10, padding: "2px 3px", borderRadius: 4, background: emp.color + "30", color: "#2D2016", fontWeight: 600, border: emp.id === sel ? `1.5px solid ${emp.color}` : "none", overflow: "hidden", lineHeight: 1.4, wordBreak: "break-all" }}>
                          <span style={{ color: emp.color, fontWeight: 700 }}>{emp.name}</span> {sh.start_time}~{sh.end_time}
                          <span style={{ fontSize: 9, color: "#8B7355" }}> ({hrs}h)</span>
                        </div>
                      );
                    })}
                  </div>
...
function ChecklistTab() {
  ...
  useEffect(() => {
    (async () => {
      const savedItems = await localGet("sm:checklist-items", { open: [], close: [] });
      const savedChecks = await localGet("sm:checklist-checks", {});
      setItems(savedItems || { open: [], close: [] });
      // 오늘 날짜가 아닌 체크는 리셋
  ...
  useEffect(() => {
    if (loaded) localSet("sm:checklist-items", items);
  }, [items, loaded]);

  useEffect(() => {
    if (loaded) localSet("sm:checklist-checks", checked);
  }, [checked, loaded]);
  ...
  const addItem = () => {
    ...
    setItems(prev => ({ ...prev, [mode]: [...prev[mode], { id, text: newItem.trim() }] }));
    setNewItem("");
  };

  const removeItem = (id) => {
    setItems(prev => ({ ...prev, [mode]: prev[mode].filter(item => item.id !== id) }));
  };
...
const TABS = [{ id: "schedule", label: "시간표", icon: "📅" }, { id: "wage", label: "급여계산", icon: "💰" }, { id: "checklist", label: "체크리스트", icon: "✅" }];
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "16px 8px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
          ...
          <div>
            ...
            <p style={{ margin: 0, fontSize: 11, color: "#B5A48B" }}>시간표 · 급여계산 · 체크리스트</p>
          </div>
        ...
        {tab !== "checklist" && <EmpPanel emps={emps} setEmps={setEmps} sel={sel} setSel={setSel} />}
        ...
        <div style={{ textAlign: "center", marginTop: 32, padding: "14px 0", borderTop: "1px solid #EDE4D8" }}>
          ...
          <p style={{ margin: "6px 0 0", fontSize: 10, color: "#D5C9B8" }}>개발자 : zee10</p>
        </div>
      </div>
    </div>
  );
