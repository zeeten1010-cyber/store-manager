...
function DailyReportTab() {
  ...
  const NumInp = ({ label, field, unit }) => (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #F5F0E8" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
        <input type="number" value={report[field]} onChange={e => u(field, e.target.value)}
          placeholder="0" style={{ width: 120, padding: "6px 10px", border: "1.5px solid #E0D5C5", borderRadius: 6, fontSize: 14, fontFamily: "inherit", textAlign: "right", outline: "none", background: "#FFFDF9" }} />
    ...
    <div>
      ...
      {loading ? (
        ...
        <React.Fragment>
          ...
          <Sec title="🏪 매장 분위기">
            <NumInp label="점심 매출" field="lunchSales" unit="원" />
            <TxtInp label="점심 분위기" field="lunchMood" placeholder="점심 시간대 분위기를 적어주세요" />
            <NumInp label="저녁 매출" field="dinnerSales" unit="원" />
...
const TABS = [{ id: "schedule", label: "시간표", icon: "📅" }, { id: "wage", label: "급여계산", icon: "💰" }, { id: "report", label: "매장관리", icon: "📋" }, { id: "checklist", label: "체크리스트", icon: "✅" }];
