...
function generateContractHTML(c) {
  const workDaysText = c.workDays.length > 0 ? c.workDays.join(", ") : "";
  const startY = c.startDate ? c.startDate.split("-")[0] : "";
  const startM = c.startDate ? c.startDate.split("-")[1] : "";
  const startD = c.startDate ? c.startDate.split("-")[2] : "";
  const endY = c.endDate ? c.endDate.split("-")[0] : "";
  const endM = c.endDate ? c.endDate.split("-")[1] : "";
  const endD = c.endDate ? c.endDate.split("-")[2] : "";
  const contractY = c.contractDate ? c.contractDate.split("-")[0] : "";
  const contractM = c.contractDate ? c.contractDate.split("-")[1] : "";
  const contractD = c.contractDate ? c.contractDate.split("-")[2] : "";
  
  // 주 소정근로시간 계산
  let weeklyHours = "";
  if (c.workStart && c.workEnd && c.workDaysCount) {
    const [sh, sm] = c.workStart.split(":").map(Number);
    const [eh, em] = c.workEnd.split(":").map(Number);
    let dailyMin = (eh * 60 + em) - (sh * 60 + sm);
    if (c.breakStart && c.breakEnd) {
      const [bsh, bsm] = c.breakStart.split(":").map(Number);
      const [beh, bem] = c.breakEnd.split(":").map(Number);
      const breakMin = (beh * 60 + bem) - (bsh * 60 + bsm);
      dailyMin -= breakMin;
    }
    const dailyH = dailyMin / 60;
    weeklyHours = (dailyH * Number(c.workDaysCount)).toFixed(1);
  }
...
body { font-family: 'Malgun Gothic', sans-serif; font-size: 10pt; line-height: 1.5; color: #000; padding: 16px; }
h1 { text-align: center; font-size: 18pt; font-weight: bold; margin-bottom: 16px; }
.intro { text-align: center; margin-bottom: 18px; font-size: 10pt; line-height: 1.6; }
.article { margin-bottom: 14px; }
.article-title { font-size: 11pt; font-weight: bold; margin-bottom: 8px; }
.content { font-size: 10pt; line-height: 1.6; padding-left: 4px; }
.content p { margin: 5px 0; }
.note { font-size: 9pt; line-height: 1.5; padding: 8px 10px; background: #f5f5f5; margin-top: 6px; border-left: 3px solid #666; }
.date-section { text-align: center; margin: 18px 0; font-size: 10pt; }
.signature-section { margin-top: 20px; }
.signature-box { margin-bottom: 16px; }
.signature-box h3 { font-size: 11pt; font-weight: bold; margin-bottom: 8px; border-bottom: 1.5px solid #000; padding-bottom: 4px; }
.signature-line { margin: 6px 0; font-size: 10pt; }
...
<p class="intro">
팔팔너구리해장(이하 "갑")과 근로자 ${c.empName ? '<strong>' + c.empName + '</strong>' : '____________'} (이하 "을")은 다음과 같이 근로계약을 체결한다.
</p>

<div class="article">
<div class="article-title">제1조 (근로계약기간)</div>
<div class="content">
<p>본 계약은 ${startY || "____"}년 ${startM || "__"}월 ${startD || "__"}일부터 ${endY || "____"}년 ${endM || "__"}월 ${endD || "__"}일까지 근로하는 것으로 한다.</p>
<p>기간이 정해지지 않으면 기간의 정함 없이 계속 근로하는 것으로 한다.</p>
</div>
</div>

<div class="article">
<div class="article-title">제2조 (근무장소 및 업무내용)</div>
<div class="content">
<p>• 근무장소: 팔팔너구리해장 신당본점</p>
<p>• 업무내용: 홀서빙, 청소 및 매장 기타(${c.extraTask || "       "})</p>
</div>
</div>

<div class="article">
<div class="article-title">제3조 (소정근로시간 및 근무일)</div>
<div class="content">
<p>• 근로시간: ${c.workStart || "__:__"} ~ ${c.workEnd || "__:__"}</p>
<p>• 근무요일: ${workDaysText || "____________"}</p>
<p>• 주 근무일수: ${c.workDaysCount || "__"}일</p>
<p>• 주 소정근로시간: 주 ${weeklyHours || "__"}시간</p>
<p>• 휴게시간: ${c.breakStart || "__:__"} ~ ${c.breakEnd || "__:__"}</p>
</div>
</div>

<div class="article">
<div class="article-title">제4조 (임금)</div>
<div class="content">
<p>• 시급: ${Number(c.wage || 0).toLocaleString()}원</p>
<p>• 임금지급일: 익월 10일</p>
<p>• 지급방법: 을의 예금계좌로 입금</p>
</div>
</div>

<div class="article">
<div class="article-title">제5조 (근무스케줄 변경)</div>
<div class="content">
<p>사업장 운영 상황에 따라 근무일 및 근로시간은 변경될 수 있으며, 이 경우 갑은 을에게 최소 1일 전까지 문자 또는 메신저 등으로 사전 통보 및 협의한다.</p>
<p>다만, 근로자의 동의 없이 주 소정근로시간을 상시적으로 변경할 수 없다.</p>
</div>
</div>

<div class="article">
<div class="article-title">제6조 (주휴수당)</div>
<div class="content">
<p>근로기준법상 주휴수당 발생시 주휴수당을 지급한다.</p>
</div>
</div>

<div class="article">
<div class="article-title">제7조 (연장근로)</div>
<div class="content">
<p>을은 갑의 요청에 따라 소정근로시간을 초과하여 근무할 수 있으며, 그 경우 임금은 관계 법령 및 사업장 기준에 따른다.</p>
<div class="note">
※ 본 사업장은 상시근로자 5인 미만 사업장으로, 연장·야간·휴일근로 가산수당은 법 적용 제외 대상임.
</div>
</div>
</div>

<div class="article">
<div class="article-title">제8조 (사회보험)</div>
<div class="content">
<p>사회보험은 관계 법령 및 가입 요건 충족 여부에 따라 적용한다.</p>
</div>
</div>

<div class="article">
<div class="article-title">제9조 (근태 및 복무)</div>
<div class="content">
<p>• 을은 근무시간을 준수하여야 하며, 지각·결근 시 사전에 통보하여야 한다.</p>
<p>• 무단결근이 반복될 경우 근로계약 해지 사유가 될 수 있다.</p>
<p>• 매장 내 재고, 금전, 시설에 대해 고의 또는 중대한 과실이 있는 경우 책임을 질 수 있다.</p>
</div>
</div>

<div class="article">
<div class="article-title">제10조 (퇴직 및 해지)</div>
<div class="content">
<p>근로자는 퇴사 시 최소 7일 전 사전 통보하여야 한다. 기타 사항은 관계 법령에 따른다.</p>
</div>
</div>

<div class="article">
<div class="article-title">제11조 (기타)</div>
<div class="content">
<p>본 계약에 명시되지 않은 사항은 근로기준법 등 관계 법령에 따른다.</p>
</div>
</div>

<div class="date-section">
<strong>계약일: ${contractY || "____"}년 ${contractM || "__"}월 ${contractD || "__"}일</strong>
</div>

<div class="signature-section">
<div class="signature-box">
<h3>[사업주]</h3>
<div class="signature-line">상호: 팔팔너구리해장</div>
<div class="signature-line">대표자: 양두환, 박지현</div>
<div class="signature-line">주소: 서울특별시 중구 퇴계로 421 1층 2호</div>
<div class="signature-line" style="margin-top: 12px;">서명: ____________</div>
</div>

<div class="signature-box">
<h3>[근로자]</h3>
<div class="signature-line">성명: ${c.empName || "____________"}</div>
<div class="signature-line">연락처: ${c.empPhone || "____________"}</div>
<div class="signature-line">주소: ${c.empAddr || "____________________________________________"}</div>
<div class="signature-line" style="margin-top: 12px;">서명: ____________</div>
</div>
</div>
