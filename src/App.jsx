                ...
                {editing ? (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
                    {allSh.map(({ emp, sh }) => {
                      ...
                      return (
                        <div key={emp.id} style={{ fontSize: 8, padding: "2px 3px", borderRadius: 4, background: emp.color + "20", color: emp.color, fontWeight: emp.id === sel ? 700 : 400, border: emp.id === sel ? `1.5px solid ${emp.color}` : "none", overflow: "hidden", whiteSpace: "nowrap", textOverflow: "ellipsis", lineHeight: 1.3 }}>
                          {emp.name} {sh.start_time}~{sh.end_time}
                          <span style={{ fontSize: 7, opacity: 0.7 }}> ({hrs}h)</span>
                        </div>
                      );
                    })}
                  </div>
